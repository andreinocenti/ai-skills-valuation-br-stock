...
def render_dividend_events(events):
    ...
    recurring = [event for event in events if event.get("classification") == "recurring"]
    extraordinary = [event for event in events if event.get("classification") == "extraordinary"]
    sample = events[-5:]
    rows = [
    ...
    rows.extend(
        f"- {event.get('date', 'sem data')}: {brl(event.get('amount_per_share'))} por acao ({event.get('classification', 'unknown')})"
        for event in sample
    )
    return lines(rows)
...
## 1. Resumo executivo
{valuation['ticker']} negocia a {brl(valuation['current_price'])}. O valor justo base estimado e {brl(valuation['fair_value_base'])}, com preco teto atual de {brl(valuation['suggested_ceiling_price'])} e preco teto projetivo de {brl(valuation['projected_ceiling_price'])}. Veredito: {valuation['verdict']}. Confianca: {valuation['confidence']}.
...
- Margem exigida: {pct(valuation['required_margin_of_safety'])}
...
## 12. Preco teto
- Preco teto por margem de seguranca: {brl(valuation['suggested_ceiling_price'])}
